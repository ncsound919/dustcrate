                                             ...
                                             juce::Slider& slider)
{
    ...
    const auto  accentDim = isSlate ? slateDim() : amberDim();
    juce::ignoreUnused(accentDim);
    ...
    juce::ignoreUnused(twoPi);

    juce::Path track;
    track.addCentredArc(cx, cy, r, r, 0.0f, startA, endA, true);
    g.setColour(knobTrack());
    g.strokePath(track, juce::PathStrokeType(2.5f, juce::PathStrokeType::curved,
                                              juce::PathStrokeType::rounded));

    juce::Path val;
    val.addCentredArc(cx, cy, r, r, 0.0f, startA, angle, true);
    g.setColour(accent);
...
{
    g.setColour(panel());        g.fillRoundedRectangle(0,0,(float)w,(float)h,3.f);
    g.setColour(panelBorder());  g.drawRoundedRectangle(.5f,.5f,w-1.f,h-1.f,3.f,1.f);
    const float ax = w - 14.f, ay = h * .5f;
    juce::Path arr;
    arr.startNewSubPath(ax, ay-2.5f); arr.lineTo(ax+5.f,ay+2.5f); arr.lineTo(ax+10.f,ay-2.5f);
    g.setColour(textSec());
    g.strokePath(arr, juce::PathStrokeType(1.2f));
}
...
void DustCrateLookAndFeel::drawLabel(juce::Graphics& g, juce::Label& l)
{
    ...
    g.fillRoundedRectangle(l.getLocalBounds().toFloat(), 2.f);
...
{
    g.setColour(panel());       g.fillRoundedRectangle(0,0,(float)w,(float)h,4.f);
    g.setColour(panelBorder()); g.drawRoundedRectangle(.5f,.5f,w-1.f,h-1.f,4.f,.8f);
}

void DustCrateLookAndFeel::drawPopupMenuItem(juce::Graphics& g,
                                              const juce::Rectangle<int>& area,
                                              bool sep, bool active, bool hi,
                                              bool, bool,
                                              ...
                                              const juce::Colour*)
{
    ...
    if (hi)  { g.setColour(rowSel()); g.fillRoundedRectangle(area.reduced(2,1).toFloat(), 3.f); }
...
void CategoryTagBar::paint(juce::Graphics& g)
{
    ...
    for (const auto& cat : categories)
    {
        ...
        juce::Rectangle<float> pill((float)x, 2.f, (float)tw, (float)ph);
        if (sel) { g.setColour(DustCrateLookAndFeel::amber()); g.fillRoundedRectangle(pill, ph*.5f); g.setColour(DustCrateLookAndFeel::body()); }
        else { g.setColour(DustCrateLookAndFeel::panelBorder()); g.drawRoundedRectangle(pill, ph*.5f, 1.f); g.setColour(DustCrateLookAndFeel::textSec()); }
...
void CategoryTagBar::mouseUp(const juce::MouseEvent& e)
{
    ...
    for (const auto& cat : categories)
    {
        ...
        {
            selectedCategory = cat; repaint();
            if (onCategorySelected) onCategorySelected(cat.equalsIgnoreCase("All") ? juce::String() : cat);
            return;
        }
        x += tw + 4;
    }
...
void SampleBrowserList::paintListBoxItem(int row, juce::Graphics& g, int w, int h, bool sel)
{
    ...
    const juce::String badge = e.category.toUpperCase().substring(0,4);
    const int bw=32, bx=w-bw-6;
    g.setColour(sel ? DustCrateLookAndFeel::amberDim() : DustCrateLookAndFeel::panelBorder());
    g.fillRoundedRectangle((float)bx, 5.f, (float)bw, (float)(h-10), 2.f);
    ...
    g.setFont(juce::Font("Helvetica Neue", 12.f, juce::Font::plain));
...
void SectionPanel::paint(juce::Graphics& g)
{
    ...
    g.setColour(DustCrateLookAndFeel::panel());       g.fillRoundedRectangle(b, 5.f);
    g.setColour(DustCrateLookAndFeel::panelBorder()); g.drawRoundedRectangle(b.reduced(.5f), 5.f, .8f);
    const auto accent = slateAccent ? DustCrateLookAndFeel::slate() : DustCrateLookAndFeel::amber();
    juce::Path top;
    top.addRoundedRectangle(b.getX(), b.getY(), b.getWidth(), 3.f, 3.f,3.f, true,true,false,false);
    g.setColour(accent); g.fillPath(top);
    g.setColour(accent);
    g.setFont(juce::Font("Helvetica Neue", 9.5f, juce::Font::bold));
    g.drawText(title, (int)b.getX()+8, (int)b.getY()+3, (int)b.getWidth()-16, kHeaderH-3,
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), mainList(p,laf), noiseList(p,laf)
{
    ...
    audioProcessor.onAudioBlock = [this](const float* d, int n)
    {
        waveform.pushSamples(d, n);
    };
    ...
    styleCombo(filterTypeCombo); filterTypeCombo.addItem("Lowpass",1); filterTypeCombo.addItem("Highpass",2); filterTypeCombo.setSelectedId(1);
                                               ...
                                               const juce::String& text, bool slate)
{
    ...
    l.setFont(juce::Font("Helvetica Neue", 9.f, juce::Font::plain));
...
void DustCrateAudioProcessorEditor::refreshBrowsers()
{
    ...
    for (const auto& e : lib.getAllSamples())
    {
        ...
        if (e.category.equalsIgnoreCase("noise")) ne.add(e); else me.add(e);
    }
...
void DustCrateAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.fillRect(0, 0, getWidth(), mh);
    g.setColour(DustCrateLookAndFeel::amber().withAlpha(0.35f));
    g.fillRect(0, mh-1, getWidth(), 1);
    ...
    g.fillRoundedRectangle((float)(getWidth()-48), 14.f, 38.f, 16.f, 3.f);
    g.setColour(DustCrateLookAndFeel::textSec());
...
void DustCrateAudioProcessorEditor::resized()
{
    ...
    auto controls = area;
    const int gap = 6;
    const int ew  = int(controls.getWidth() * 0.30f);
    const int fw  = int(controls.getWidth() * 0.28f);
    ...
    auto placeKnobs = [](juce::Rectangle<int> panelBounds,
                         std::initializer_list<std::pair<juce::Slider*, juce::Label*>> items)
    {
        auto inner = panelBounds.reduced(4);
        inner.removeFromTop(SectionPanel::kHeaderH + 2);
        const int n = (int) items.size();
        const int sw2 = inner.getWidth() / n;
        for (auto& [s, l] : items)
        {
            auto col = inner.removeFromLeft(sw2);
    ...
    placeKnobs(cb, { {&noiseLevelSlider,&noiseLabelKnob},{&driftSlider,&driftLabel},{&vhsSlider,&vhsLabel},{&cassetteSlider,&cassetteLabel} });
